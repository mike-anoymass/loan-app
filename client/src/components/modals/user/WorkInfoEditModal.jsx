...
function WorkInfoEditModal({ isOpen, onClose, data, setData, loanID}) {
  ...
  const [formData, setFormData] = useState({
    incomeSource: data.workInfo ? data.workInfo.incomeSource : "",
    description: data.workInfo ? data.workInfo.description : "",
    workplace: data.workInfo ? data.workInfo.workplace: "",
  })

  const editWorkInfo = () => {
    setLoading(true)
    axios.post("http://localhost:3001/workInfo", formData, {
      headers: {
      ...
      <div className={modalClass}>
        <div className="bg-white rounded-lg overflow-hidden md:w-1/4 w-2/3">
          <div className="p-4">
            ...
            <h2 className='text-lg font-semibold p-2'>Modify Employment Information</h2>
            <hr />

            <div className='p-2 flex flex-col'>
              <label htmlFor="" className='mt-4'>Income Source</label>
              <input type="text" value={formData.incomeSource} placeholder='Employed, Business, Pension' className={formClass}
              ...
              <label htmlFor="" className='mt-4'>Workplace address</label>
              <input type="text" value={formData.workplace} className={formClass}
                onChange={e => setFormData({...formData, workplace:e.target.value})} 
              />

              <button 
                className='w-full mt-3 border-2 p-2 bg-green-600 text-white rounded-lg'
                onClick={() => editWorkInfo()}
...
export default WorkInfoEditModal